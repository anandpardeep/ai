...
class PDFExtractor(Extractor):   
    ...
    def detect_columns(self, elements: List[ContentElement], page: fitz.Page) -> List[ContentElement]:
        """Use pymupdf4llm column_boxes to assign column numbers to elements."""
        try:
    ...
    def detect_columns1(self, elements: List[ContentElement], page_width: float) -> List[ContentElement]:
        if not elements:
            return elements
        
        # Get x-coordinates of element centers
        x_centers = [elem.center_x for elem in elements]
        
        if len(set(x_centers)) < 2:
            # Single column
            for elem in elements:
                elem.column = 0
            return elements
        ...
        threshold = page_width * 0.15  # 15% of page width as threshold
        ...
        for elem in elements:
            elem_x = elem.center_x
            for col_idx, col_x_coords in enumerate(columns):
                if any(abs(elem_x - x) < threshold/2 for x in col_x_coords):
                    elem.column = col_idx
                    break
            else:
                elem.column = 0  # Default to first column
